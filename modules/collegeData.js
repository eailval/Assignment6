...
const sequelize = new Sequelize(
  "tysofzvq",
  "tysofzvq",
  "sdO5zs2UPBSUR-IBRLo7JxFGlo4l4AM5",
  {
...
module.exports.updateStudent = async function (studentData) {
  try {
    const result = await db.Student.update(studentData, { where: { studentNum: studentData.studentNum } });
    if (result[0] === 0) {
      throw new Error(`Could not find student with studentNum ${studentData.studentNum}`);
    }
  } catch (error) {
    ...
    db.Course.findOne({ where: { courseId: courseData.courseId } })
      .then((course) => {
        ...
        db.Course.update(courseData, {
          ...
          .catch((err) => {
            reject(new Error("Unable to update course"));
          });
      })
      .catch((err) => {
        reject(new Error("Unable to find course"));
      });
  });
};
